...
def check_permission_rules(tool_name: str, args: dict) -> str | None:
    for rule in PERMISSION_RULES:
        if tool_name in rule["tools"] and rule["rule"](args):
            return rule["message"]
    return None
...
def check_permission(block):
    ...
    reason = check_permission_rules(block.name, block.input)
    if reason:
